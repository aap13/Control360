...
function distribuicao_setup_import_tables(PDO $pdo): void
{
    ...
    distribuicao_ensure_column($pdo, 'distribuicao_equipamentos', 'empresa', "VARCHAR(150) DEFAULT NULL AFTER cliente_id");
...
{
    $statusColumn = distribuicao_clientes_status_column($pdo);
    $whereAtivo = $statusColumn . ' = 1';

    if ($isAdmin) {
        $stmt = $pdo->query("SELECT id, nome FROM distribuicao_clientes WHERE {$whereAtivo} ORDER BY nome");
...
function distribuicao_clientes_status_column(PDO $pdo): string
{
    static $cache = null;
    if ($cache !== null) {
        return $cache;
    ...
    $stmt = $pdo->prepare("
        ...
        FROM information_schema.COLUMNS
        WHERE TABLE_SCHEMA = :schema
          AND TABLE_NAME = 'distribuicao_clientes'
          AND COLUMN_NAME IN ('ativo', 'status')
        ORDER BY FIELD(COLUMN_NAME, 'ativo', 'status')
        LIMIT 1
    ");
...
function distribuicao_parse_base_csv(string $filePath, int $clienteId): array
{
    ...
    $aliases = [
        'cliente_nome' => ['cliente_nome', 'empresa', 'empresa_nome'],
        'codigo_local' => ['codigo_local', 'codigo', 'cod_local'],
        'pp' => ['pp'],
        'uf' => ['uf'],
        'municipio' => ['municipio', 'cidade'],
        'regional' => ['regional'],
        'setor_unidade' => ['setor_unidade', 'setor', 'unidade', 'local'],
        'logradouro' => ['logradouro', 'endereco'],
        'bairro' => ['bairro'],
        'cep' => ['cep'],
        'cnpj' => ['cnpj'],
        'centro_custo' => ['centro_custo', 'centro_de_custo', 'cc'],
        'tipo_equipamento' => ['tipo_equipamento', 'tipo'],
        'fabricante' => ['fabricante', 'manufacturer'],
        'modelo' => ['modelo', 'model'],
        'serie' => ['serie', 'serial'],
        'nome_impressora' => ['nome_impressora', 'printer', 'nome'],
        'status_operacional' => ['status_operacional', 'status'],
        'data_instalacao' => ['data_instalacao', 'instalacao'],
    ...
    foreach ($aliases as $canonical => $options) {
        foreach ($options as $option) {
            if (in_array($option, $headers, true)) {
                $map[$canonical] = $option;
                break;
            }
        }
    }

    $required = ['cliente_nome', 'municipio', 'setor_unidade', 'modelo', 'serie'];
    $missing = [];
    ...
    foreach ($lines as $line) {
        ...
        $value = static function (string $field) use ($data, $map): string {
            if (!isset($map[$field])) {
                return '';
            }
            return trim((string) ($data[$map[$field]] ?? ''));
        };
        ...
        $rows[] = [
            ...
            'empresa' => $value('cliente_nome'),
            'codigo_local' => $value('codigo_local'),
            'pp' => $value('pp'),
            'uf' => strtoupper($value('uf')),
            'municipio' => $value('municipio'),
            'regional' => $value('regional'),
            'setor' => $value('setor_unidade'),
            'logradouro' => $value('logradouro'),
            'bairro' => $value('bairro'),
            'cep' => $value('cep'),
            'cnpj' => $value('cnpj'),
            'centro_custo' => $value('centro_custo'),
            'tipo' => $value('tipo_equipamento'),
            'fabricante' => $value('fabricante'),
            'modelo' => $value('modelo'),
            'serie' => $serie,
            'nome_impressora' => $value('nome_impressora'),
            'status_operacional' => $value('status_operacional') ?: 'Ativa',
            'data_instalacao' => distribuicao_parse_date($value('data_instalacao')),
...
function distribuicao_importar_base_csv(PDO $pdo, array $rows, int $userId): array
{
    ...
    $insert = $pdo->prepare("
        INSERT INTO distribuicao_equipamentos (
            cliente_id, empresa, codigo_local, pp, uf, municipio, regional, setor,
    ...
    try {
        foreach ($rows as $row) {
            ...
            if ($existingId > 0) {
                $updateParams = $params;
                unset($updateParams[':cliente_id'], $updateParams[':serie']);
