...
<!-- Visão geral -->
<div class="stats-grid chamados-stats distribuicao-stats">
    <article class="stat-card chamados-stat chamados-stat-total">
        <div class="sc-label">Total de impressoras</div>
        <div class="sc-value"><?= number_format((float) $stats['total'], 0, ',', '.') ?></div>
        <div class="sc-foot">Base consolidada do parque</div>
        <div class="sc-bar"></div>
    </article>
    <article class="stat-card chamados-stat chamados-stat-success">
        <div class="sc-label">Ativas</div>
        <div class="sc-value"><?= number_format((float) $overviewCounts['ativas'], 0, ',', '.') ?></div>
        <div class="sc-foot">Em operação</div>
        <div class="sc-bar"></div>
    </article>
    <article class="stat-card chamados-stat chamados-stat-danger">
        <div class="sc-label">Offline</div>
        <div class="sc-value"><?= number_format((float) $overviewCounts['offline'], 0, ',', '.') ?></div>
        <div class="sc-foot">Sem leitura recente</div>
        <div class="sc-bar"></div>
    </article>
    <article class="stat-card chamados-stat chamados-stat-neutral">
        <div class="sc-label">Backup</div>
        <div class="sc-value"><?= number_format((float) $overviewCounts['backup'], 0, ',', '.') ?></div>
        <div class="sc-foot">Reserva técnica</div>
        <div class="sc-bar"></div>
    </article>
</div>
...
(() => {
    ...
    const monitorColors = ['#16a34a', '#dc2626', '#6b7280'];
    ...
    if (monitorCanvas && monitorData.some(v => v > 0)) {
        new Chart(monitorCanvas, {
            ...
            data: {
                ...
                datasets: [{ data: monitorData, backgroundColor: monitorColors, borderColor: '#090b0f', borderWidth: 4, hoverOffset: 8 }]
            },
            options: {
                responsive: true,
                maintainAspectRatio: false,
