...
echo render_flash();
?>
<div class="page-head"><div class="page-head-copy"><h2>Inventário de celulares</h2><p>Centralize linha móvel, IMEI, operadora e status de MDM em uma visualização consistente com o restante do sistema.</p></div><?php if ($canWrite): ?><div class="page-head-actions"><a href="cadastrar.php?tipo=celular" class="btn btn-primary"><?= icon('plus') ?> Novo celular</a></div><?php endif; ?></div>

<div class="inventory-overview-grid">
    <div class="inventory-kpi-card">
        <span class="inventory-kpi-label">Total</span>
        <strong><?= number_format((float) $statsCelulares['total'], 0, ',', '.') ?></strong>
        <small>Celulares listados</small>
    </div>
    <div class="inventory-kpi-card">
        <span class="inventory-kpi-label">Em uso</span>
        <strong><?= number_format((float) $statsCelulares['em_uso'], 0, ',', '.') ?></strong>
        <small>Status operacional</small>
    </div>
    <div class="inventory-kpi-card">
        <span class="inventory-kpi-label">Disponíveis</span>
        <strong><?= number_format((float) $statsCelulares['disponivel'], 0, ',', '.') ?></strong>
        <small>Prontos para uso</small>
    </div>
    <div class="inventory-kpi-card">
        <span class="inventory-kpi-label">MDM ativo</span>
        <strong><?= number_format((float) $statsCelulares['mdm_ativo'], 0, ',', '.') ?></strong>
        <small>Gerenciamento habilitado</small>
    </div>
</div>
